...
def test_get_volume_from_pressure_birch_murnaghan():
    ...
    from aiida_sssp_workflow.workflows.convergence.pressure import (
        helper_get_volume_from_pressure_birch_murnaghan,
    )
...
def test_phonon_frequencies_diff():
    ...
    from aiida_sssp_workflow.workflows.convergence.phonon_frequencies import (
        helper_phonon_frequencies_difference,
    )

    input_parameters = {"dynamical_matrix_0": {"frequencies": [1.0, 1.0, 1.0]}}
    ref_parameters = {"dynamical_matrix_0": {"frequencies": [1.0, 1.0, 1.0]}}

    res = helper_phonon_frequencies_difference(
        orm.Dict(dict=input_parameters), orm.Dict(dict=ref_parameters)
    )
