...
def get_structure_from_element(element='Si'):
    ...
    structure = orm.CifData.get_or_create(
        cif_file)[0].get_structure(primitive_cell=True)

    return structure

def run_band_eva(code, pseudos, ecutwfc=30.0, ecutrho=120.0):
    inputs = {
        'code': code,
        'pseudos': pseudos,
        ...
        'init_nbands_factor': orm.Float(3),
        'options': orm.Dict(
                dict={
                    'resources': {
                        'num_machines': 1
    ...
    res, node = run_get_node(BandsWorkChain, **inputs)
...
if __name__ == '__main__':
    ...
    pseudos = {'Si': pseudo}
    res, node = run_band_eva(code, pseudos)
