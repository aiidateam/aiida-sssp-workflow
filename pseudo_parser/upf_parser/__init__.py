...
REGEX_Z_VALENCE_V1 = re.compile(r"""(?P<z_valence>""" + PATTERN_FLOAT +
                                r""")\s+Z valence""")
REGEX_Z_VALENCE_V2 = re.compile(
    r"""\s*z_valence\s*=\s*['"]\s*(?P<z_valence>""" + PATTERN_FLOAT +
    r""")\s*['"].*""")
...
def parse_z_valence(content: str) -> int:
    ...
    raise ValueError(
        f'could not parse the Z valence from the UPF content: {content}')
...
def parse_pseudo_type(content: str) -> str:
    ...
    raise ValueError(
        f'could not parse the pseudo_type from the UPF content: {content}')
